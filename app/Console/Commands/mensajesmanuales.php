...
class mensajesmanuales extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Mensajes:manuales';
    ...
    /*public function handle()
    {
      //selecciono todos los clientes
        ...
        foreach ($cli as $moroso) {
          $txt="actualmente presenta un saldo deudor de: ".number_format($moroso->deuda, 2)." Bs.S. Le invitamos a realizar el pago y a reportarlo para seguir disfrutando del servicio.";
          ...
          if (!$fp) {
            ...
            fwrite($fp, "Action: smscommand\r\ncommand: gsm send sms 2 ".urlencode($moroso->phone1)." \"".urlencode($message)."\" ".rand()." \r\n\r\n");
    ...
    public function handle()
    {
        ...
        foreach ($cli as $moroso) {
            if ($moroso->tipo_planes == 2 ||$moroso->tipo_planes == 5 ) {
                $txt = "se ha generado una nueva factura correspondiente al mes en curso, su saldo actual es de: " . number_format($moroso->deuda, 2) . " US$, reporte su pago a traves de info@maraveca.com, https://maraveca.com/mi-ventana";//cambio del formato del link de mi pago a mi ventana 07/06/2019,
