...
class factura extends Command
{
     ...
     * @var string
     */
    protected $signature = 'factura:generar
  ...
  {nro_servicio?}';
    ...
    public function handle()
    {
        ...
        if($this->argument('fecha2')){
            $today2 = Carbon::createFromFormat('d/m/Y', $this->argument('fecha2') , 'America/Caracas');
            $fecha2 = $today;
        }else{
            $today2 = new Carbon('first day of next month');
            $fecha2 = $today;
        }
        ...
        $nbOfDaysRemainingThisMonth =  $today->diff($today2)->format('%a');
        ...
        foreach ($clientes as $cliente) { //para cada cliente
            ...
            $tasa2=configuracion::where('nombre','=','taza(BCV)')->first()->valor;
            ...
            if ($servicios->count()  >= 1 || $adicionales->count()>=1) { //en caso de que exista al menos un servicio
                            ...
                            if($pro == 1){
                                ...
                                DB::table('fac_products')->insert(
                                    [
                                        ...
                                        'IVA'=>$iva->valor,
                                        'comment_articulo'=>$key->comment_srv.", Fac: Prorrateada (".$this->argument('fecha')."-".$this->argument('fecha2').")",
                                        "created_at" =>  \Carbon\Carbon::now(), # \Datetime()
                                        "updated_at" => \Carbon\Carbon::now(),  # \Datetime()
                                        "precio_dl" =>round(((($key->cost_plan/$totalofdays)*$nbOfDaysRemainingThisMonth)*$iva1)/$tasa, 2),
                                        "precio_bs" => round(((($key->taza/$totalofdays)*$nbOfDaysRemainingThisMonth)*$tasa2), 2),
                                    ]
                                );
                            }else{
                                ...
                                DB::table('fac_products')->insert(
                                    [
                                        ...
                                        'precio_bs' =>round(($key->taza *$tasa2),2),
                            ...
                            if($pro==1){
                                ...
                                DB::table('fac_products')->insert(
                                    [
                                        ...
                                        'IVA'=>$iva->valor,
                                        'comment_articulo'=>$key->comment_srv.", Fac: Completa",
                                        "created_at" =>  \Carbon\Carbon::now(), # \Datetime()
                                        "updated_at" => \Carbon\Carbon::now(),  # \Datetime()
                              ...
                              /*  if($key->carac_plan== 2){
                                ...
                                DB::table('fac_products')->insert(
                                    [
                                        ...
                                        'precio_articulo'=>$key->taza,
                                        'IVA'=>$iva->valor,
                                        'comment_articulo'=>$key->comment_srv.", Fac: Completa (".$fechaFactura.")",
                                        "created_at" =>  \Carbon\Carbon::now(), # \Datetime()
                                        "updated_at" => \Carbon\Carbon::now(),  # \Datetime()
                                        'precio_dl'=>round((($key->cost_plan )/$tasa),2),
                                        'precio_bs' =>round(($key->taza *$tasa2),2),
             ...
             historico_cliente::create(['history'=>'Generacion de factura Nº '.$id." Fecha: ".$fechaFactura, 'modulo'=>'Facturacion', 'cliente'=>$cliente->id, 'responsable'=>$responsable]);
